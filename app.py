# imports
import os
# imports joblib so i can load the saved vectorizer, model and threshold files
import joblib
# imports pandas so i can create, read and update the csv review log
import pandas as pd
# imports numpy because i need argmax and array handling for DistilBERT outputs
import numpy as np
# imports torch because the DistilBERT model runs with pytorch
import torch
# imports streamlit because the entire front end is built with streamlit
import streamlit as st
# imports datetime so every saved review row gets a timestamp
from datetime import datetime
# imports the tokenizer loader for the saved DistilBERT folder
from transformers import AutoTokenizer
# imports the sequence classification model loader for the saved DistilBERT folder
...
# FILE PATHS
#  folder where the saved model files are stored
SAVE_DIR = "frontend_assets"
# subfolder that contains the saved DistilBERT model and tokenizer files
DISTILBERT_DIR = os.path.join(SAVE_DIR, "DistilBERT")
# filename for the csv log that stores reviewer feedback
LOG_FILE = "moderation_log.csv"

# LABEL HELPERS

# standardises prediction labels so they always display consistently across old and new rows
def clean_prediction_label(value):
    # converts the value to text, strips spaces and lowercases it for easy comparison
    text = str(value).strip().lower()
    # returns the standard toxic label
    if text == "toxic":
        return "Toxic"
    # catches all previous non-toxic variants saved in different formats
    if text in ["non-toxic", "non toxic", "nontoxic"]:
        return "Non-Toxic"
    # falls back to the original text if nothing else matches
    return str(value).strip()


# standardises review labels so they display neatly in the audit table
def clean_review_label(value):
    # handles empty or missing values without throwing an error
    if pd.isna(value):
        return ""

    # replaces underscores with spaces, strips whitespace and title-cases the result
...
# caches all model resources so streamlit does not reload them on every interaction
@st.cache_resource
def load_assets():
    # loads the saved tf-idf vectorizer used to turn raw text into features
    vectorizer = joblib.load(os.path.join(SAVE_DIR, "tfidf_vectorizer.joblib"))

    # loads the saved calibrated logistic regression model
    model = joblib.load(os.path.join(SAVE_DIR, "calibrated_model.joblib"))

    # loads the saved best threshold chosen in the notebook
    threshold = joblib.load(os.path.join(SAVE_DIR, "best_threshold.joblib"))

    # loads the DistilBERT tokenizer from the saved folder
    distilbert_tokenizer = AutoTokenizer.from_pretrained(DISTILBERT_DIR)

    # loads the DistilBERT classification model from the saved folder
    distilbert_model = AutoModelForSequenceClassification.from_pretrained(DISTILBERT_DIR)

    # switches DistilBERT to evaluation mode so dropout layers are disabled during inference
    distilbert_model.eval()

    # returns all loaded objects as a tuple
    return vectorizer, model, threshold, distilbert_tokenizer, distilbert_model


# unpacks all loaded model resources into named variables
vectorizer, model, saved_threshold, distilbert_tokenizer, distilbert_model = load_assets()

# LOG SETUP

# creates the csv log file if it does not already exist on disk
if not os.path.exists(LOG_FILE):
    # builds an empty dataframe with the correct column names for a fresh log
    ...
    # writes the empty dataframe to disk as the starting review log
    empty_log.to_csv(LOG_FILE, index=False)

# PREDICTION FUNCTIONS

# runs the tf-idf baseline on one comment and returns a probability and label
def predict_comment_tfidf(text, threshold):
    # vectorises the raw text using the saved tf-idf transformer
    text_vec = vectorizer.transform([text])

    # gets the class 1 probability from the calibrated classifier
    prob_toxic = model.predict_proba(text_vec)[0][1]

    # applies the threshold to produce a binary decision
    prediction = 1 if prob_toxic >= threshold else 0

    # converts the numeric decision into a human-readable label
    label = "Toxic" if prediction == 1 else "Non-Toxic"

    # returns the probability score and the label
    return prob_toxic, label


# runs the saved DistilBERT model on one comment and returns a probability and label
def predict_comment_distilbert(text, threshold):
    ...
    # disables gradient computation because no training is happening here
    with torch.no_grad():
        # passes the tokenised input through the DistilBERT model
        outputs = distilbert_model(**encoded)

        # extracts the raw logit scores from the model output
        logits = outputs.logits

        # converts the logits into a probability distribution across classes
        probs = torch.softmax(logits, dim=1)

        # extracts the probability for the toxic class
        prob_toxic = probs[0, 1].item()

    # applies the threshold to produce a binary decision
    prediction = 1 if prob_toxic >= threshold else 0

    #converts the numeric decision to a label
    label = "Toxic" if prediction == 1 else "Non-Toxic"

    # returns the probability and the label
    return prob_toxic, label

# LOG SAVE FUNCTION

# appends one reviewed case to the persistent csv audit log
def save_review(comment_text, model_used, prob_toxic, threshold, prediction, review_label, notes):
    # builds a single-row dataframe containing all fields for the review record
    row = pd.DataFrame(
        [
            {
    ...
    # appends the row to the existing csv file without rewriting the header
    row.to_csv(LOG_FILE, mode="a", header=False, index=False)

# LOAD LOG FOR DISPLAY

#     reads the existing log from disk so the audit pane can display history
log_df = pd.read_csv(LOG_FILE)

# normalises prediction labels in old rows so display is consistent
if "prediction" in log_df.columns:
    log_df["prediction"] = log_df["prediction"].apply(clean_prediction_label)

# normalises review labels in old rows so display is consistent
if "review_label" in log_df.columns:
    log_df["review_label"] = log_df["review_label"].apply(clean_review_label)

# counts how many logged rows were classified as toxic
toxic_count = 0
if not log_df.empty:
    toxic_count = (log_df["prediction"] == "Toxic").sum()

# counts how many logged rows were classified as non-toxic
non_toxic_count = 0
if not log_df.empty:
...
# builds the left sidebar with model selection and threshold controls
with st.sidebar:
    # renders the sidebar section header
    st.markdown("## ⚙️ Settings")

    # adds a divider line under the header
    st.markdown("---")

    # this adds a small label above the model selector
    st.markdown("**Select Model**")

    # creates a dropdown so the reviewer can switch between the baseline and DistilBERT
    ...
    # adds vertical spacing between the two controls
    st.markdown(" ")

    # adds a label above the threshold slider
    st.markdown("**Decision Threshold**")

    # creates the threshold slider and initialises it at the saved notebook value
    threshold = st.slider(
        "Decision Threshold",
        min_value=0.05,
        max_value=0.99,
    ...
    # shows the currently selected threshold value below the slider
    st.markdown(
    ...
    # adds a divider and a short guidance note at the bottom of the sidebar
    st.markdown("---")
    st.markdown(
...
# renders the branded navy blue header banner at the top of the main area
st.markdown(
    """
...
# creates the main two-column layout: left for the review workflow, right for the audit pane
main_col, audit_col = st.columns([1.85, 1], gap="large")

# LEFT COLUMN — REVIEW WORKFLOW

# starts the wider left column containing the main moderation cards
with main_col:

    # Input card 

    # renders the section heading for the input card using the custom card style
    st.markdown(
    ...
    # shows which model is currently active so the reviewer knows what they are using
    st.markdown(
    ...
    # uses a form so the textarea and submit button are submitted together in one action
    with st.form("prediction_form"):
        # creates the multi-line comment input area with a school-appropriate placeholder
        ...
        # renders the analyse button inside the form
        submitted = st.form_submit_button("🔍  Analyse Comment", use_container_width=True)

    # closes the input section card div
    st.markdown("</div>", unsafe_allow_html=True)

    # Run prediction

    # runs only when the user clicks the analyse button
    if submitted:
        # shows a warning if the comment box was submitted empty
        if not comment_text.strip():
            st.warning("Please enter a comment before analysing.")
        else:
            # runs the tf-idf baseline if it is selected
            if model_choice == "TF-IDF + Calibrated Classifier":
                prob_toxic, prediction_label = predict_comment_tfidf(comment_text, threshold)

            # runs the DistilBERT model if it is selected
            elif model_choice == "DistilBERT":
                prob_toxic, prediction_label = predict_comment_distilbert(comment_text, threshold)

            # saves the current comment into session state so it persists after the page reruns
            st.session_state["latest_comment"] = comment_text

            # saves the toxic probability into session state
            st.session_state["latest_prob"] = prob_toxic

            # saves the prediction label into session state
            st.session_state["latest_prediction"] = prediction_label

            # saves the threshold used at prediction time into session state
            st.session_state["latest_threshold"] = threshold

            # saves which model produced the result into session state
    ...
    if "latest_comment" in st.session_state:
        ...
        # retrieves the stored prediction values from session state
        ...
        # picks the badge HTML based on whether the comment was flagged or not
        badge_html = (
        ...
        # sets the colour for the confidence bar: red for toxic, green for safe
        bar_colour = "#e74c3c" if pred_label == "Toxic" else "#27ae60"

        # calculates the percentage width for the confidence bar
        bar_width = int(pred_prob * 100)

        # renders the prediction result card with badge, metric boxes and confidence bar
        st.markdown(
            f"""
            <div class='section-card'>
        ...
        # renders the reviewer feedback section heading
        st.markdown(
        ...
        # creates the feedback form so the review decision is saved in one step
        with st.form("review_form"):
            # creates the review outcome radio buttons displayed horizontally
            ...
            # creates the notes text area for the reviewer to add context
            notes = st.text_area(
                "Notes (optional)",
                height=90,
            ...
            # renders the save button for the review form
            review_submitted = st.form_submit_button("💾  Save Review Decision", use_container_width=True)

        # closes the feedback card div
        st.markdown("</div>", unsafe_allow_html=True)

        # saves the review to the log when the reviewer submits the feedback form
        if review_submitted:
            # calls the save function with all relevant session and form values
            save_review(
            ...
            # shows a confirmation message so the reviewer knows the row was saved
            st.success("✅ Review decision saved to the audit log.")


# RIGHT COLUMN - AUDIT AND ANALYTICS PANE

#     starts the narrower right column for the audit history and summary counts
with audit_col:

    # renders the audit pane section header
    st.markdown(
        "<div class='section-card'><h3>📋 Audit and Analytics Pane</h3>",
        unsafe_allow_html=True
    )

    # creates three side-by-side columns for the summary count boxes
    c1, c2, c3 = st.columns(3)

    # shows the total number of toxic decisions in the log
    with c1:
        st.markdown(
    ...
    # shows the total number of non-toxic decisions in the log
    with c2:
        st.markdown(
    ...
    # shows the total number of rows in the log
    with c3:
        st.markdown(
    ...
    # adds a small gap after the summary boxes
    st.markdown("<div style='margin-top:0.8rem;'></div>", unsafe_allow_html=True)

    # renders the audit table if there are saved review rows
    if not log_df.empty:
        # creates a display copy of the log so the original dataframe stays unchanged
        display_log_df = log_df.copy()

        # maps internal column names to cleaner display names for the table
        rename_map = {
            "timestamp": "Time",
        ...
        # applies the column rename to the display copy
        display_log_df = display_log_df.rename(columns=rename_map)

        # builds the list of columns to show, only including ones that exist
        wanted_cols = [
            col for col in
        ...
        # keeps only the wanted display columns
        display_log_df = display_log_df[wanted_cols]

        # rounds the probability column to four decimal places for a cleaner table
        if "Probability" in display_log_df.columns:
            display_log_df["Probability"] = display_log_df["Probability"].round(4)

        # renders the audit dataframe table, expanding to fill the column width
        st.dataframe(display_log_df, use_container_width=True)

        # encodes the full raw log as utf-8 csv bytes for the download button
        csv_bytes = log_df.to_csv(index=False).encode("utf-8")

        # renders a download button so reviewers can export the full audit log as csv
        st.download_button(
        ...
        # shows a placeholder message when the log has no rows yet
        st.info("No review history yet. Save a review decision to populate this panel.")

    # closes the audit section card div
    st.markdown("</div>", unsafe_allow_html=True)


# PAGE FOOTER

# renders a small footer note at the bottom of the page with project attribution
st.markdown(
